...
class BasicObject {
	 ...
	 */
	static function treatParameters(&$parameters, $setup) {
		if (!is_array($setup))
			return;
		...
		foreach ($setup as $parameterName => $parameterSetup) {
			if (!isset($parameters[$parameterName])) {
				if ($parameterSetup["isRequired"] ?? false) {
					...
					$e->Errors->trigger(Errors::ERROR_SYSTEM, ["errorDescription" => "Parameter \"".$parameterName."\" is required when calling ".debug_backtrace()[1]["class"]."::".debug_backtrace()[1]["function"]]);
				}
