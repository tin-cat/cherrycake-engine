...
class Request {
	...
	const PARAMETER_TYPE_FILES = 3;
	const PARAMETER_TYPE_CLI = 4;
	...
	function __construct(
		...
		if ($this->isSecurityCsrf()) {
			$this->parameters[] = new \Cherrycake\Modules\Actions\RequestParameter(
				name: "csrfToken",
	...
	function retrieveParameterValues(): bool {
		...
		if ($this->pathComponents) {
			foreach ($this->pathComponents as $index => $pathComponent) {
				if(
					$pathComponent->type == \Cherrycake\Modules\Actions\Request::PATH_COMPONENT_TYPE_VARIABLE_STRING
					||
					$pathComponent->type == \Cherrycake\Modules\Actions\Request::PATH_COMPONENT_TYPE_VARIABLE_NUMERIC
				) {
	...
	function buildUrl(
		...
		if ($this->parameters && $isIncludeUrlParameters) {
			foreach ($this->parameters as $parameter) {
				if ($parameter->type !== \Cherrycake\Modules\Actions\Request::PARAMETER_TYPE_GET)
					continue;
				if ($parameterValues ?? false) {
